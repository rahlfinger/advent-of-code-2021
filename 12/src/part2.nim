...
proc findPaths(caveMap: seq[string], pathMap: seq[string]): seq[string] =
    ...
    # See if we already found our double small cave
    var hasDoubleSmallCave = false

    for option in pathMap:
        if isUpperAscii(option[0]) == false:
            let opt = option
            let instances = filter(pathMap, str => opt == str)
            if instances.len > 1:
    ...
    for option in nextStepOptionsNoStarts:
        ...
        if isUpperAscii(nextStep[0]) == true:
            ...
            if isAlreadyTraveled.len == 1 and hasDoubleSmallCave == false:
                cleanedOptions.add(option)
...
echo (fmt"Part 2 answer: {totalPoints.len}")
